...
router.get('/', function(req, res) {
	conn.query(
        ...
        SELECT DISTINCT `P`.`ProductID`, `P`.`ProductName`, `P`.`AddedDate`, `C`.`CategoryName`, (SELECT SUM(`PVF`.`Quantity`) FROM `ProductsVariants` `PV` INNER JOIN `ProductsVariantsFlavors` `PVF` ON `PV`.`VariantID` = `PVF`.`VariantID` WHERE `PV`.`ProductID` = `P`.`ProductID` AND `PV`.`Deleted` = 0 AND `PVF`.`Deleted` = 0) AS `Quantity` FROM `Products` `P` INNER JOIN `Categories` `C` ON `P`.`CategoryID` = `C`.`CategoryID` WHERE `P`.`Deleted` = 0 ORDER BY `P`.`AddedDate` DESC;\
